"""
Simple KnowYourMeme View Count Updater using requests (faster than Selenium)
"""
...
def extract_views(soup: BeautifulSoup) -> int:
    ...
    try:
        # Method 1: Look for the aside stats section
        aside = soup.select_one('aside')
        if aside:
            text = aside.get_text()
            # Look for patterns like "12,345,678 Views"
            match = re.search(r'([\d,]+)\s*Views', text, re.IGNORECASE)
            if match:
                return int(match.group(1).replace(',', ''))

        # Method 2: Look in any text for "Views" pattern
        page_text = soup.get_text()
        matches = re.findall(r'([\d,]+)\s*Views', page_text, re.IGNORECASE)
        if matches:
            # Return the largest number (likely the total views)
            views_list = [int(m.replace(',', '')) for m in matches]
...
def scrape_view_count(session: requests.Session, url: str) -> int:
...
def main():
    ...
    try:
        for i, meme in enumerate(memes, 1):
            ...
            if meme_id not in MEME_URLS:
                print(f"[{i}/{len(memes)}] {meme['name']}: No URL mapping")
                no_url_count += 1
                continue

            url = MEME_URLS[meme_id]
            print(f"[{i}/{len(memes)}] {meme['name']}...", end=" ", flush=True)

            views = scrape_view_count(session, url)
            ...
            if views:
                ...
                print(f"FAILED (keeping {meme['views']:,})")
                failed_count += 1
    ...
    print("-" * 50)
    print(f"Done! Updated: {updated_count}, Failed: {failed_count}, No URL: {no_url_count}")
