"""
KnowYourMeme View Count Updater v2
Correctly extracts view counts from the sidebar stats (glasses icon = views)
"""
...
# Mapping of meme IDs to their KnowYourMeme search terms or URLs
MEME_SEARCH_TERMS = {
    "doge": "doge",
    ...
    "confused-math-lady": "confused math lady",
...
def search_kym(session: requests.Session, search_term: str) -> str:
    ...
    try:
        ...
        # KYM search results have links to /memes/ pages
        for link in soup.select('a[href*="/memes/"]'):
            href = link.get('href', '')
            if '/memes/' in href and not '/memes/people/' in href and not '/memes/sites/' in href:
                if href.startswith('/'):
...
def extract_views_from_page(soup: BeautifulSoup) -> int:
    ...
    The views are shown next to a glasses icon in the sidebar stats.
    Format: glasses icon followed by number like "14,589,339"
    """
    try:
        # Method 1: Look for the entry stats in the aside/sidebar
        # The stats section has icons followed by numbers
        # Glasses icon = views, play button = videos, folder = images, speech bubble = comments

        # Find the aside or sidebar section
        aside = soup.select_one('aside')
        if aside:
            # Get all text and look for large numbers (views are typically millions)
            aside_html = str(aside)

            # The view count appears after specific patterns
            # Look for numbers with commas that are likely view counts
            numbers = re.findall(r'(\d{1,3}(?:,\d{3})+|\d+)', aside.get_text())

            # Filter for numbers that look like view counts (> 1000)
            view_candidates = []
            for num_str in numbers:
                try:
                    num = int(num_str.replace(',', ''))
                    if num > 1000:  # Views are typically > 1000
                        view_candidates.append(num)
                except:
                    pass

            # The largest number in the aside is usually the view count
            if view_candidates:
                return max(view_candidates)

        # Method 2: Look for specific stat elements
        # KYM uses dd elements or specific classes for stats
        stat_elements = soup.select('.stat, .views, dd')
        for elem in stat_elements:
            text = elem.get_text().strip()
            match = re.search(r'([\d,]+)', text)
            if match:
                num = int(match.group(1).replace(',', ''))
                if num > 10000:  # Likely a view count
                    return num

        # Method 3: Search the entire page for the pattern near "views" text
        page_text = soup.get_text()
        # Look for numbers followed by or preceded by "views"
        matches = re.findall(r'([\d,]+)\s*(?:views|Views)', page_text)
        if matches:
            views = [int(m.replace(',', '')) for m in matches]
            return max(views)
...
def scrape_meme_views(session: requests.Session, meme_url: str) -> int:
    ...
    try:
        ...
        print(f"Error scraping {meme_url}: {e}")
...
def main():
    ...
    try:
        for i, meme in enumerate(memes, 1):
            ...
            # Get search term - use mapping or fall back to meme name
            ...
            if views and views > 1000:
                ...
                print(f"NO VIEWS FOUND (keeping {meme['views']:,})")
                failed_count += 1
